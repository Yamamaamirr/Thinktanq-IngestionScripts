"""
Step 7 - CDL candidate generation for California (tiled).

CA CDL raster is 954M pixels — scipy.ndimage.label() would need 7.6 GB int64 array.
...
Intermediate outputs per tile in candidate_areas/data/cdl/ca_tiles/
Final outputs:
  candidate_areas/outputs/states/ca/cdl_candidates_ca.parquet
  candidate_areas/outputs/states/ca/cdl_candidates_ca.fgb

Run: python candidate_areas/Step7_cdl_ca.py
"""
...
CDL_PATH  = Path('candidate_areas/data/cdl/source/ca/CDL_2025_06.tif')
KZF_PATH  = Path('candidate_areas/outputs/keep_zones_final.parquet')
OUT_DIR   = Path('candidate_areas/data/cdl')
TILE_DIR  = OUT_DIR / 'ca_tiles'
OUT_PARQUET = Path('candidate_areas/outputs/states/ca/cdl_candidates_ca.parquet')
OUT_FGB     = Path('candidate_areas/outputs/states/ca/cdl_candidates_ca.fgb')
...
print(f'\n{"="*55}')
print(f' CDL CANDIDATE GENERATION - CA (tiled {TILE_ROWS}x{TILE_COLS})')
print(f'{"="*55}')
...
checks = {
    ...
    'CA centroids'    : result.centroid_lat.between(32, 42).all(),
}
all_pass = True
...
print('\nVerify cdl_candidates_ca.fgb in QGIS.')
